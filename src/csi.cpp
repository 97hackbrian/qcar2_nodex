...
int main(int argc, char ** argv)
{
    ...
    param_desc.description = "The IP address of the device (only used for virtual)";
    node->declare_parameter("ip", "localhost", param_desc);
    ip = node->get_parameter("ip").as_string();
    ...
    if (device_type.compare("physical")==0 ){
        ...
        if (sprintf(uri, "video://%s:%i@tcpip://%s:%i", ip.c_str(), camera_num, ip.c_str(), 18961+camera_num) < 0)
        {
            RCLCPP_ERROR(node->get_logger(), "Cannot form camera uri.");
            return -1;
        }
